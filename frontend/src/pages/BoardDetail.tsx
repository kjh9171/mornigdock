...
import { User, Clock, Eye, MessageSquare, CornerDownRight, Trash2, ArrowLeft, Send, ExternalLink, Bot, ShieldCheck, Sparkles } from 'lucide-react'
      ...
      <div className="bg-white border border-stone-100 rounded-2xl p-5 shadow-sm relative group hover:shadow-md transition-all">
        ...
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[11px] font-black uppercase tracking-tight ${comment.author_name.includes('Admin') ? 'text-amber-600' : 'text-stone-800'}`}>
        ...
        <div className="p-10 border-b border-stone-100">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black px-3 py-1 rounded-full bg-primary-100 text-primary-700 uppercase tracking-widest">
                {post.source || 'INTERNAL'}
              </span>
              {post.type === 'news' && (
                <span className="text-[10px] font-black px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 uppercase tracking-widest flex items-center gap-1">
            ...
            {post.source_url && (
              <a href={post.source_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-stone-50 text-stone-600 rounded-xl text-xs font-bold hover:bg-stone-100 transition-all border border-stone-200 shadow-sm">
        ...
        {post.ai_analysis ? (
          <div className="p-10 bg-stone-900 border-t border-stone-800">
            <div className="flex items-center gap-2 mb-6 text-amber-400">
              ...
              <h3 className="text-xl font-black tracking-tight uppercase">AI Strategic Analysis</h3>
            </div>
            <div className="bg-stone-800/50 p-8 rounded-3xl border border-stone-700 shadow-inner relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-10 rotate-12"><Bot className="w-24 h-20" /></div>
              <pre className="text-sm text-stone-300 whitespace-pre-wrap font-sans leading-relaxed relative z-10 italic">
                {post.ai_analysis}
              </pre>
            </div>
          </div>
        ) : post.type === 'news' && (
          <div className="p-10 bg-stone-50 border-t border-stone-100 flex justify-center">
            <button 
              onClick={handleAIAnalysis}
              className="flex items-center gap-3 px-8 py-4 bg-stone-900 text-white rounded-2xl font-black text-sm hover:bg-black transition-all shadow-xl hover:shadow-amber-600/20"
            >
