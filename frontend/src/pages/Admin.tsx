        ...
        <div className="w-full space-y-8 animate-in fade-in duration-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-2 text-[10px] font-black text-stone-400 uppercase tracking-widest"><Globe className="w-3 h-3" /> AGORA WIDE-SCREEN COMMAND <span className="text-stone-200">|</span> v1.0.6</div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="w-full">
              ...
              {tab === 'users' && (
                <Card title="요원 레지스트리 및 프로토콜" icon={Shield} action={<button onClick={() => setUserModal({ open: true })} className="flex items-center gap-2 text-[10px] font-black bg-stone-900 text-white px-5 py-2.5 rounded-xl uppercase tracking-widest hover:bg-black transition-all shadow-lg"><UserPlus className="w-4 h-4" /> 신규 요원 배치</button>}>
                  <div className="overflow-x-auto -mx-8"><table className="w-full text-sm"><thead><tr className="text-[10px] font-black text-stone-400 uppercase border-b border-stone-100 bg-stone-50/30"><th className="py-5 px-8 text-left whitespace-nowrap">식별 ID / 호출명</th><th className="py-5 px-8 text-left whitespace-nowrap">보안 권한</th><th className="py-5 px-8 text-center whitespace-nowrap">상태</th><th className="py-5 px-8 text-right whitespace-nowrap">전략 통제</th></tr></thead><tbody className="divide-y divide-stone-50">
                    {users.map(u => (
                      <tr key={u.id} className="hover:bg-stone-50/50 transition-all"><td className="py-6 px-8 whitespace-nowrap"><span className="font-black text-stone-800 block uppercase tracking-tight">{u.username}</span><span className="text-[10px] text-stone-400 font-mono italic">{u.email}</span></td><td className="py-6 px-8 whitespace-nowrap"><span className={`text-[10px] font-black px-2.5 py-1 rounded-lg uppercase ${u.role === 'admin' ? 'bg-stone-900 text-amber-400' : 'bg-stone-100 text-stone-600'}`}>{u.role}</span></td><td className="py-6 px-8 text-center whitespace-nowrap"><span className={`text-[9px] font-black px-3 py-1.5 rounded-full uppercase ${u.is_active ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>{u.is_active ? '정상 작동' : '접속 차단'}</span></td><td className="py-6 px-8 text-right whitespace-nowrap"><div className="flex justify-end gap-2"><button onClick={() => setUserModal({ open: true, item: u })} className="p-3 text-stone-400 hover:text-stone-900 hover:bg-white rounded-2xl shadow-sm transition-all border border-transparent hover:border-stone-200"><Edit2 className="w-4 h-4" /></button><button onClick={() => handleDeleteUser(u.id)} disabled={u.id === user.id} className="p-3 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-2xl transition-all"><Trash2 className="w-4 h-4" /></button></div></td></tr>
                    ))}
                  </tbody></table></div>
                </Card>
              )}
              ...
              {tab === 'posts' && (
                <Card title="인텔리전스 분석물 통제" icon={FileText}>
                  <div className="overflow-x-auto -mx-8"><table className="w-full text-sm"><thead><tr className="text-[10px] font-black text-stone-400 uppercase border-b border-stone-100 bg-stone-50/30"><th className="py-5 px-8 text-left whitespace-nowrap">분류</th><th className="py-5 px-8 text-left whitespace-nowrap">리포트 제목</th><th className="py-5 px-8 text-left whitespace-nowrap">분석관</th><th className="py-5 px-8 text-right whitespace-nowrap">통제</th></tr></thead><tbody className="divide-y divide-stone-50">
                    {posts.map(p => (
                      <tr key={p.id} className={`hover:bg-stone-50/50 transition-all ${p.pinned ? 'bg-amber-50/30' : ''}`}><td className="py-6 px-8 whitespace-nowrap"><span className={`text-[9px] font-black px-2.5 py-1 rounded-lg uppercase ${p.type === 'news' ? 'bg-blue-100 text-blue-700' : 'bg-stone-100 text-stone-500'}`}>{p.type}</span></td><td className="py-6 px-8 max-w-[600px]"><div className="flex items-center gap-2">{p.pinned && <Pin className="w-3 h-3 text-amber-600" />}<span className="text-stone-800 font-black truncate leading-tight tracking-tight">{p.title}</span></div></td><td className="py-6 px-8 text-stone-400 font-bold text-[10px] uppercase whitespace-nowrap">{p.author_name}</td><td className="py-6 px-8 text-right whitespace-nowrap"><div className="flex justify-end gap-1"><button onClick={() => adminDeletePostAPI(p.id).then(() => loadData())} className="p-3 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-2xl transition-all"><Trash2 className="w-4 h-4" /></button></div></td></tr>
                    ))}
                  </tbody></table></div>
                </Card>
              )}
