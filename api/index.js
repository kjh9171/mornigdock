...
var news = pgTable("news", {
  ...
  // AI Analysis
  generationalReaction: text("generational_reaction"),
  // Generation-specific reaction
...
var newsRelations = relations(news, ({ many }) => ({
  comments: many(comments)
}));
...
var DatabaseStorage = class {
  ...
  async createUser(data) {
    ...
    const [user] = await this.db.select().from(users).where(eq(users.username, data.username));
    return user;
  }
  async updateUserMfa(userId, secret, enabled) {
    await this.db.update(users).set({ mfaSecret: secret, isMfaEnabled: enabled }).where(eq(users.id, userId));
  }
  // =========================
  // News
  // =========================
  ...
  async createNews(data) {
    ...
    const [item] = await this.db.select().from(news).orderBy(desc(news.id));
    return item;
  }
  async updateNewsAnalysis(id, analysis) {
    await this.db.update(news).set({
  ...
  async createComment(data) {
    ...
    const [comment] = await this.db.select().from(comments).orderBy(desc(comments.id));
    return comment;
  }
  // =========================
  // Media
  ...
  async createMedia(data) {
    ...
    const [item] = await this.db.select().from(media).orderBy(desc(media.id));
    return item;
  }
  // =========================
  // Stats (D1-safe)
  // =========================
  async getStats() {
    const usersCountResult = await this.db.select({ count: sql`count(*)` }).from(users);
    const newsCountResult = await this.db.select({ count: sql`count(*)` }).from(news);
    return {
...
// server/index.ts
function log(message, source = "worker") {
