...
export const fetchNewsService = async () => {
  ...
  const newsItems = [
    {
      ...
      source: '네이버 뉴스 (연합뉴스)',
      source_url: 'https://n.news.naver.com/mnews/article/001/0015223123', // 코스피 속보 링크
    ...
    },
    {
      ...
      source: '네이버 뉴스 (연합뉴스)',
      source_url: 'https://n.news.naver.com/mnews/article/001/0015223789', // 삼성전자 단독 링크
      author_name: '네이버 뉴스 스크래퍼'
    },
    {
      type: 'news',
      ...
      source: '네이버 뉴스 (연합뉴스)',
      source_url: 'https://n.news.naver.com/mnews/article/001/0015223456', // 현대차 수소차 링크
      author_name: '네이버 뉴스 스크래퍼'
    }
  ]
  ...
  for (const item of newsItems) {
    ...
    if (exists.rows.length === 0) {
      ...
      // 🔥 [데이터 정제] 제목은 같지만 URL이 꼬인 경우를 대비해 상시 업데이트
      await pool.query(
        `UPDATE posts SET source_url = $1, content = $2 WHERE title = $3`,
        [item.source_url, item.content, item.title]
      )
    }
  }
  
  console.log('✅ CERT: Naver News Intelligence Scraped and Synchronized.')
}
