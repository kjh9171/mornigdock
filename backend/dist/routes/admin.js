...
admin.get('/dashboard', adminOnly, async (c) => {
    ...
    // 최근 접속 로그 20건 조회
    const recentLogs = await query(`SELECT al.*, u.name AS user_name
     FROM access_logs al
...
admin.get('/inquiries', adminOnly, async (c) => {
    const result = await query(`SELECT i.*, u.name as user_name, u.email as user_email
     FROM inquiries i
...
admin.get('/users', adminOnly, async (c) => {
    const result = await query(`SELECT u.id, u.email, u.name, u.role, u.is_blocked, u.otp_enabled,
...
admin.post('/users', adminOnly, async (c) => {
    ...
    const result = await query(`INSERT INTO users (email, name, password, role)
     VALUES ($1, $2, $3, $4)
     RETURNING id, email, name, role, is_blocked, created_at`, [email, name, hashed, role]);
...
admin.post('/media', adminOnly, async (c) => {
    ...
    const res = await query(`INSERT INTO media (title, type, url, description, thumbnail, artist, is_featured, is_active, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW()) RETURNING *`, [title, type, url, description || '', autoThumbnail || '', artist || '', is_featured, is_active]);
...
admin.put('/media/:id', editorOnly, async (c) => {
    ...
    params.push(id);
    const result = await query(`UPDATE media SET ${setClauses.join(', ')} WHERE id = $${params.length} RETURNING *`, params);
...
admin.put('/settings', adminOnly, async (c) => {
    ...
    await Promise.all(entries.map(([key, value]) => query(`INSERT INTO system_settings (key, value, updated_at)
         VALUES ($1, $2, NOW())
...
admin.get('/posts', adminOnly, async (c) => {
    const result = await query(`SELECT p.*, u.name as author_name
     FROM posts p
