...
comments.get('/', optionalAuth(), async (c) => {
    ...
    const result = await query(`SELECT c.id, c.parent_id, c.content, c.created_at, c.updated_at,
            u.id AS user_id, u.name AS user_name, u.role AS user_role
     FROM comments c
     JOIN users u ON u.id = c.user_id
     ${whereClause}
     ORDER BY c.created_at ASC`, [idValue]);
    // 계층형 트리 구성
...
comments.post('/', requireAuth(), async (c) => {
    ...
    const result = await query(`INSERT INTO comments (news_id, post_id, user_id, parent_id, content)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, content, parent_id, created_at`, [newsId ?? null, postId ?? null, userId, parentId ?? null, content]);
...
comments.put('/:id', requireAuth(), async (c) => {
    ...
    if (!content)
        return c.json({ success: false, message: '내용이 필요합니다.' }, 400);
