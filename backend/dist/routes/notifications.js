...
notifications.get('/', async (c) => {
    try {
        ...
        const stockResult = await pool.query("SELECT name, price, change_val, change_rate, updated_at FROM stocks WHERE symbol IN ('KOSPI', 'KOSDAQ') ORDER BY symbol ASC");
        ...
        // 2. 최신 뉴스 3건 가져오기
        const newsResult = await pool.query("SELECT id, title, published_at FROM news ORDER BY published_at DESC LIMIT 3");
