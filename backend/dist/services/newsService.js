    ...
    if (naverClientId && naverClientSecret) {
        try {
            ...
            for (const item of items) {
                try {
                    await query(`INSERT INTO news (title, description, url, source_name, category, published_at)
             VALUES ($1, $2, $3, $4, $5, $6)
        ...
        try {
            ...
            for (const item of items) {
                ...
                try {
                    await query(`INSERT INTO news (title, description, url, source_name, category, published_at)
             VALUES ($1, $2, $3, $4, $5, $6)
             ON CONFLICT (url) DO NOTHING`, [title, description, link, feed.name, feed.category, pubDate ? new Date(pubDate).toISOString() : new Date().toISOString()]);
                    savedCount++;
                }
                catch (e) { }
            }
