...
export const fetchStockService = async () => {
    ...
    try {
        ...
        for (const el of newsItems) {
            ...
            if (check.rows.length === 0) {
                ...
                await pool.query(`INSERT INTO news (title, description, url, source_name, category, published_at)
           VALUES ($1, $2, $3, $4, $5, NOW())
           RETURNING id`, [title, analysis.summary, link, '네이버 금융', 'finance']);
                // 상세 분석 결과 저장
                const lastNewsId = await pool.query('SELECT id FROM news WHERE url = $1', [link]);
                await pool.query(`INSERT INTO ai_reports (news_id, summary, impact, advice)
           VALUES ($1, $2, $3, $4)`, [lastNewsId.rows[0].id, analysis.summary, analysis.impact, analysis.advice]);
            }
        }
    ...
    for (const item of stockItems) {
        try {
            await pool.query(`INSERT INTO stocks (symbol, name, price, change_val, change_rate, market_status, ai_summary, updated_at) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
         ON CONFLICT (symbol) DO UPDATE SET 
         name = EXCLUDED.name, price = EXCLUDED.price, change_val = EXCLUDED.change_val, 
         change_rate = EXCLUDED.change_rate, ai_summary = EXCLUDED.ai_summary, updated_at = NOW()`, [item.symbol, item.name, item.price, item.change_val, item.change_rate, item.market_status, item.ai_summary]);
        }
        catch (e) {
